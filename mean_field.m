function mean_field(nx, ny, k_m, k_i, frac, field, x_cut, y_cut)
% =========================================================================
% CALCULATION OF THE EFFECTIVE THERMAL CONDUCTIVITY AND TEMPERATURE FIELDS
% COMPARING LEHT (ANALYTICAL) AND FVT - MEAN FIELD (NUMERICAL)
% =========================================================================
clc;
...
% Material Design
x = InitialMaterialDesign(nx, ny, l, h, L, H, R, k_m, k_i);
...
K_ast_FVT = G0 * mean(x(:)) + [x(:)' * Tx; x(:)' * Ty] / (nx*ny);
...
kcase = 1; 
if Hx_plot==0 && Hy_plot==1, kcase = 2; end

[I_mesh,J_mesh] = ndgrid(1:nx,1:ny);
I_vec = I_mesh(:);
J_vec = J_mesh(:);
Xfaces = [(I_vec-0.5)*l, I_vec*l, (I_vec-0.5)*l, (I_vec-1)*l];
Yfaces = [(J_vec-1)*h, (J_vec-0.5)*h, J_vec*h, (J_vec-0.5)*h];
Ttil_case = reshape(T_til(faces(:),kcase), nx*ny, 4);
Ttot_faces = Xfaces*Hx_plot + Yfaces*Hy_plot + Ttil_case;
